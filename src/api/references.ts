...
function cleanVehicleReferenceTitle(result: GlobalSearchResult) {
  ...
  // Reference sources usually index the real car name, not the listing year or marketplace suffix.
  ...
  if (brand) {
    ...
    if (index >= 0) {
      ...
      const modelTokens = suffix
        .split(/\s+/)
        .filter(token => !/^\d{5,}$/.test(token))
        .filter(token => !/^(?:car|vehicle|automobile|render|scene)$/i.test(token))
        .slice(0, 6)
      value = [brand, ...modelTokens].join(' ').trim()
    } else if (value && !value.toLowerCase().startsWith(brand.toLowerCase())) {
...
function referenceCandidates(result: GlobalSearchResult) {
  ...
  const clean = cleanVehicleReferenceTitle(result)
  const noYearRaw = raw.replace(YEAR_RE, ' ').replace(/\s+/g, ' ').trim()
  const seen = new Set<string>()
  return [raw, clean, noYearRaw]
...
export async function getReferencePack(result: GlobalSearchResult, signal?: AbortSignal): Promise<ReferencePack> {
  ...
  for (let index = 0; index < candidates.length; index += 1) {
    ...
    const pack = await requestReferencePack(result, title, index === 0, signal)
    lastPack = pack
