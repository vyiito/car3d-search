...
import {
  ...
  Columns3, Database, Download, FileText, Filter, Gamepad2, Gauge, Globe2, Grid2X2, Heart,
...
type SortMode = 'Mix de fontes' | 'Relevância' | 'Mais recentes' | 'Nome A-Z' | 'Fonte'
...
function mixBySource(items: GlobalSearchResult[]) {
  ...
  const mixed: GlobalSearchResult[] = []
  let index = 0
  while (mixed.length < items.length) {
    let added = false
    for (const group of orderedGroups) if (group[index]) { mixed.push(group[index]); added = true }
    if (!added) break
    index += 1
  }
  return mixed
...
function App() {
  ...
  const gameCounts = useMemo(() => { const map=new Map<string,number>(); rawResults.forEach(r=>{if(r.game) map.set(r.game,(map.get(r.game)||0)+1)}); return [...map.entries()].sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0])) }, [rawResults])
  ...
  useEffect(() => {
    setSelectedSources(cur=>cur.filter(s=>sourceCounts.some(([name])=>name===s)))
    setSelectedFormats(cur=>cur.filter(f=>formatCounts.some(([name])=>name===f)))
    setSelectedGames(cur=>cur.filter(g=>gameCounts.some(([name])=>name===g)))
  }, [sourceCounts,formatCounts,gameCounts])
  ...
  const filteredResults = useMemo(() => {
    ...
    let items=rawResults.filter(result => {
      ...
      const matchesGames=!selectedGames.length||Boolean(result.game&&selectedGames.includes(result.game))
      ...
      return matchesKind&&matchesVehicle&&matchesFormats&&matchesSources&&matchesGames&&matchesDownload&&matchesImage&&matchesYear
    })
    ...
    if(sortMode==='Mix de fontes') return mixBySource(items)
  ...
  }, [rawResults,kind,vehicleClass,selectedFormats,selectedSources,selectedGames,downloadMode,imageMode,yearFrom,yearTo,sortMode,groupDuplicates])
  ...
  const activeFilters=[kind!=='Todos',vehicleClass!=='Todos',selectedFormats.length>0,selectedSources.length>0,selectedGames.length>0,downloadMode!=='Todos',imageMode!=='Todos',Boolean(yearFrom),Boolean(yearTo),sortMode!=='Mix de fontes',groupDuplicates].filter(Boolean).length

  const clearFilters=()=>{setKind('Todos');setVehicleClass('Todos');setSelectedFormats([]);setSelectedSources([]);setSelectedGames([]);setDownloadMode('Todos');setImageMode('Todos');setYearFrom('');setYearTo('');setSortMode('Mix de fontes');setGroupDuplicates(false)}

  const runSearch=(term=query, brand:string|null=null, game:string|null=null)=>{
    const clean=term.trim(); if(clean.length<2) return
    setQuery(clean); setSubmittedQuery(clean); setSelectedBrand(brand); setSelectedGames(game?[game]:[]); setSearchFocused(false)
    ...
    const params=new URLSearchParams(); if(brand) params.set('brand',brand); else params.set('q',clean); if(game) params.set('game',game); window.history.pushState({},'',`${window.location.pathname}?${params}`)
  ...
  return <main>
    ...
    <header className="topbar"><a className="brand" href="#"><span className="brandMark">VJ</span><span>3D SEARCH<small>FREE AUTOMOTIVE META INDEX</small></span></a><nav><a href="#search">BUSCAR</a><a href="#brands">MARCAS</a><a href="#games">JOGOS</a><a href="#results">CATÁLOGO</a><a href="#sources">FONTES</a><a className="githubLink" href="https://github.com/vyiito/car3d-search" target="_blank" rel="noreferrer">GITHUB <ArrowUpRight size={13}/></a></nav><span className="topStatus"><i/> ONLINE · {providers.length} FONTES</span></header>

    <section className="hero" id="search"><div className="heroCode">VJ // AUTOMOTIVE SEARCH SYSTEM <span>01</span></div><div className="eyebrow"><ShieldCheck size={14}/> APENAS VEÍCULOS · APENAS GRÁTIS</div><h1>ENCONTRE O <em>3D</em><br/><span>QUE ESTÁ FALTANDO.</span></h1><p>Pesquise por modelo, geração, ano, marca ou jogo de origem. O VJ consolida assets gratuitos automotivos e percorre múltiplas páginas das fontes compatíveis.</p>
    ...
    {!submittedQuery&&<><DiscoveryCarousel onSelect={openResult} onSearch={term=>runSearch(term)}/><BrandsExplorer onSearch={brand=>runSearch(brand,brand)} recent={brandHistory}/><GamesExplorer onSearch={game=>runSearch(game,null,game)}/>{favorites.length>0&&<section className="savedSection"><div className="savedHead"><span><Heart size={13}/> FAVORITOS / LOCAL</span><b>{favorites.length}</b></div><div className="savedRail">{favorites.slice(0,10).map(item=><button key={item.id} onClick={()=>openResult(item)}>{item.imageUrl?<img src={item.imageUrl} alt=""/>:<span className="savedFallback"><CarFront size={18}/></span>}<strong>{item.title}</strong><small>{item.source}{item.game?` · ${item.game}`:''}</small></button>)}</div></section>}</>}
    ...
    <section className={`resultsShell ${!submittedQuery?'searchIdle':''}`} id="results">
      ...
      <div className="content"><aside className={`filtersPanel ${filtersOpen?'open':''}`}><div className="filterTitle"><SlidersHorizontal size={17}/><div><strong>REFINAR BUSCA</strong><small>{activeFilters?`${activeFilters} filtro(s) ativo(s)`:'facetas desta pesquisa'}</small></div><button onClick={()=>setFiltersOpen(false)} className="filterClose"><X size={16}/></button></div>
        ...
        {gameCounts.length>0&&<div className="filterSection sourcesFacetSection"><span className="filterSectionTitle"><Gamepad2 size={13}/> JOGOS NESTA BUSCA <small>{selectedGames.length||''}</small></span><div className="sourceFacets gameFacets">{gameCounts.map(([name,count])=><button key={name} className={selectedGames.includes(name)?'active':''} onClick={()=>setSelectedGames(cur=>toggleValue(cur,name))}><span className="facetCheck">{selectedGames.includes(name)&&<Check size={11}/>}</span><span className="facetName">{name}</span><b>{count}</b></button>)}</div></div>}
        {formatCounts.length>0&&<div className="filterSection"><span className="filterSectionTitle"><FileText size={13}/> FORMATOS <small>{selectedFormats.length||''}</small></span><div className="facetChips">{formatCounts.slice(0,16).map(([name,count])=><button key={name} className={selectedFormats.includes(name)?'active':''} onClick={()=>setSelectedFormats(cur=>toggleValue(cur,name))}><span>{name}</span><b>{count}</b></button>)}</div></div>}
