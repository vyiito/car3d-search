...
const UA = 'VJ3DSearch/3.0 (+https://github.com/vyiito/car3d-search)'
...
const ANGLES = [
  { id: 'front', label: 'FRONT', terms: 'front view' },
  { id: 'rear', label: 'REAR', terms: 'rear view' },
  { id: 'side', label: 'SIDE', terms: 'side view' },
  { id: 'three-quarter', label: '3/4', terms: 'three quarter view' },
  { id: 'interior', label: 'INTERIOR', terms: 'interior dashboard' },
  { id: 'details', label: 'DETAILS', terms: 'wheel detail' },
]
...
async function searchOpenverse(baseQuery, angle, perAngle = 5) {
  const url = new URL('https://api.openverse.org/v1/images/')
  url.searchParams.set('q', `${baseQuery} ${angle.terms}`)
  url.searchParams.set('page_size', String(Math.max(3, Math.min(perAngle + 3, 12))))
  ...
  for (const item of rows) {
    ...
    results.push({
      ...
      title: clean(item?.title) || `${baseQuery} ${angle.label}`,
      imageUrl,
      thumbnailUrl,
      sourcePage,
      ...
      downloadAllowed: openverseAllowed(item),
      redistributionNote: openverseAllowed(item) ? 'Licença aceita pelo VJ Reference Pack.' : 'Referência apenas; não entra no ZIP automático.',
    })
...
async function searchCommons(baseQuery, angle, perAngle = 3) {
  const url = new URL('https://commons.wikimedia.org/w/api.php')
  url.searchParams.set('action', 'query')
  url.searchParams.set('format', 'json')
  url.searchParams.set('generator', 'search')
  url.searchParams.set('gsrsearch', `${baseQuery} ${angle.terms}`)
  url.searchParams.set('gsrnamespace', '6')
  url.searchParams.set('gsrlimit', String(Math.max(3, Math.min(perAngle + 2, 8))))
  ...
  for (const page of pages) {
    ...
    results.push({
      ...
      angle: angle.id,
      angleLabel: angle.label,
      title: clean(String(page?.title || '').replace(/^File:/i, '')) || `${baseQuery} ${angle.label}`,
      imageUrl,
      thumbnailUrl,
      sourcePage,
...
export async function searchReferencePack(asset, options = {}) {
  ...
  const batches = await Promise.all(ANGLES.map(async angle => {
    try {
      const openverse = await searchOpenverse(baseQuery, angle, perAngle)
      if (openverse.length >= Math.min(3, perAngle)) return openverse
      const commons = await searchCommons(baseQuery, angle, Math.max(1, perAngle - openverse.length))
      return [...openverse, ...commons]
    } catch {
      try { return await searchCommons(baseQuery, angle, perAngle) } catch { return [] }
    }
  }))
  const images = dedupe(batches.flat()).slice(0, 36)
...
export function getReferencePack(packId) {
  trimCache()
  const entry = cache.get(String(packId || ''))
...
export async function buildReferenceZip(packId, selectedIds = []) {
  ...
  if (!pack) throw new Error('Reference pack expired or not found.')
  const wanted = new Set(selectedIds.filter(Boolean))
  const candidates = pack.images.filter(image => image.downloadAllowed && (!wanted.size || wanted.has(image.id))).slice(0, MAX_PACK_IMAGES)
  if (!candidates.length) throw new Error('No redistributable reference images selected.')
  const entries = [], manifest = [], angleCounters = new Map()
  let totalBytes = 0
  for (const image of candidates) {
    try {
      const fetched = await readImage(image.imageUrl)
      if (totalBytes + fetched.buffer.length > MAX_PACK_BYTES) break
      totalBytes += fetched.buffer.length
      const count = (angleCounters.get(image.angle) || 0) + 1
      angleCounters.set(image.angle, count)
      const folder = safeFilePart(image.angleLabel || image.angle).toUpperCase()
      const name = `${folder}/${String(count).padStart(2, '0')}_${safeFilePart(image.title)}.${fetched.extension}`
      entries.push({ name, data: fetched.buffer })
      manifest.push([
        name,
        `Title: ${image.title}`,
        `Angle: ${image.angleLabel}`,
        `Creator: ${image.creator}`,
        `License: ${image.license}${image.licenseVersion ? ` ${image.licenseVersion}` : ''}`,
        `License URL: ${image.licenseUrl || 'Not provided'}`,
        `Source page: ${image.sourcePage}`,
        `Original image: ${image.imageUrl}`,
      ].join('\n'))
    } catch (error) {
      manifest.push(`SKIPPED: ${image.title}\nReason: ${error instanceof Error ? error.message : 'download failed'}\nSource page: ${image.sourcePage}`)
    }
  }
